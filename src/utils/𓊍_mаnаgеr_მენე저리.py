...
class 레이어_관리_시스템:   # אבג あいう ሀ ሀ ለ क kh
    def __init__(self, ሐ_canvas):
        self.ሐ = ሐ_canvas
        # ᄠᅳᆮ : 층의 자료를 자바두ᄂᆞᆫ 것이라 (옛한글)
        # א_data = layer_data, あ_list = layer_list
        self.א_data = {"레이어_1": {"visible": True, "locked": False, "opacity": 1.0}}
        self.あ_list = ["레이어_1"] 
        self.ሀ_현 = "레이어_1"    # ᄠᅳᆮ : 이제 쓰ᄂᆞᆫ 층이라 (옛한글)
        self.ለ_count = 1         #ሀ ሀ ለ क kh

    def 추가_레이어_액션(self):
        # ᄠᅳᆮ : ᄉᆡ 층을 ᄆᆡᆼᄀᆞᄂᆞᆫ 것이라 (옛한글)
        self.ለ_count += 1
    ...
    def 앞으로_가져오기_Z(self, obj):
        # ᄠᅳᆮ : 도형을 ᄆᆡᆫ 위로 올리ᄂᆞᆫ 것이라 (옛한글)
        try: self.ሐ.tag_raise(obj)
        except: pass

    def 뒤로_보내기_Z(self, obj):
        # ᄠᅳᆮ : 도혀형으를 ᄆᆡᆫ 아ᄅᆡ로 내리ᄂᆞᆫ 것이ᄅ라 (옛한글)
