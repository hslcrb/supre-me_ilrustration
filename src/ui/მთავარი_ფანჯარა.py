...
class 畫板App:
    def __init__(self):
        ...
        self.티ლო = tk.Canvas(self.메인_공간, bg="white")
        
        self.역사_현황 = 역사_현황_시스템(self.티ლო)
        self.저장_도구 = 저장_유틸_최고(self.티ლო)
        ...
        მენიუ.add_cascade(label="파일", menu=파일_메뉴)
        파일_메뉴.add_command(label="저장", command=self.저장_도구.저장_프로세스)
        파일_메뉴.add_command(label="불러오기", command=self.저장_도구.로드_프로세스)
        파일_메뉴.add_command(label="종료", command=self.rооt.quit)
        
        편집_메뉴 = tk.Menu(მენიუ, tearoff=0)
        მენიუ.add_cascade(label="편집", menu=편집_메뉴)
        편집_메뉴.add_command(label="실행 취소", command=self.역사_현황.undo_취소)
        편집_메뉴.add_command(label="다시 실행", command=self.역사_현황.redo_다시)
        
        캔버스_메뉴 = tk.Menu(მენიუ, tearoff=0)
        მენიუ.add_cascade(label="캔버스", menu=캔버스_메뉴)
        캔버스_메뉴.add_command(label="배경색", command=self.배경_도구.색상_변경)
        ...
        tk.Button(self.공구상자, text="브러시", command=self.모드_자유, bg="#ccffcc").pack(side=tk.LEFT, padx=2)
        tk.Button(self.공구상자, text="상자", command=lambda: self.모드_형상("rect"), bg="#e0e0e0").pack(side=tk.LEFT, padx=2)
        tk.Button(self.공구상자, text="원", command=lambda: self.모드_형상("oval"), bg="#e0e0e0").pack(side=tk.LEFT, padx=2)
        tk.Button(self.공구상자, text="선", command=lambda: self.모드_형상("line"), bg="#e0e0e0").pack(side=tk.LEFT, padx=2)
        tk.Button(self.공구상자, text="텍스트", command=self.모드_문자, bg="#ccccff").pack(side=tk.LEFT, padx=2)
    ...
    def 그리드_토글_액션(self):
        ...
        if self.그리드_상태:
            for i in range(0, 1400, 20):
                self.티ლო.create_line(i, 0, i, 850, fill="#eeeeee", tags="grid")
            for i in range(0, 850, 20):
                self.티ლო.create_line(0, i, 1400, i, fill="#eeeeee", tags="grid")
